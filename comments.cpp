...
//стандартный вектор от Ильвохина. Примечание: почищено от ненужного
template <typename T>
class TVector {
public:
    using value_type = T;
...
void CountingSort(TVector <TInsex>& a){     //каунтинг сорт. передаем вектор по ссылке, чтобы его менять в самой функции и мэйне.
  int *c = new int[SIZE_ARRAY]{0};    // создаем вспомогательный массив и заполняем его нулями
  TVector <TInsex> out(a.Size());  // это вектор размером с входной для промежуточной записи отсортированного
...
int main(){
    ...
    while (scanf("%llu%llu", &k, &e) == 2) {     // не знаю, почему, но scanf работает быстрее. Возможно, это связано с тем, что cin и cout должны синхронизироваться со стандартной библиотекой Си
      tmp.Set(k, e);
      a.PushBack(tmp);     //суем в вектор
    }

    CountingSort(a);  //сортируем и а сразу сортируется в то же а
